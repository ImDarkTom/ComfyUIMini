...
function loadServerWorkflowMetadata(workflowsFolder, jsonFileList) {
    ...
    for (const jsonFilename of jsonFileList) {
        ...
        if (!jsonMetadata) {
            ...
            logger.info(`Created auto-generated ComfyUIMini metadata for '${jsonFilename}', to disable this feature you can disable it in config.`);
...
function autoGenerateMetadata(workflowJson, jsonFilename, workflowsFolder) {
    ...
    fs.writeFileSync(path.join(workflowsFolder, `[CONVERTED] ${jsonFilename}`), JSON.stringify(fullWorkflowJson, null, 2), 'utf8');
